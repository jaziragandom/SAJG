...
import { 
  Building2, Target, Users, Award, MapPin, Download, X, ExternalLink, 
  ChevronLeft, ChevronRight, Mail, Phone, Globe, Leaf, ShieldCheck, Factory, MapPinned, MessageCircle,
  Zap, Activity, Send, Briefcase, CheckCircle2 
} from "lucide-react";
...
function Counter({ value, title }: { value: number; title: string }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center justify-center py-4 px-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-700 hover:border-amber-400 transition-colors">
      ...
      <p className="text-gray-600 dark:text-gray-400 text-xs font-bold text-center mt-1">{title}</p>
    </div>
  );
}

// --- تنظیمات نمایش ---
const visibilitySettings = { showPartners: true, showCertificates: true };

// --- دیتای بخش‌ها ---
const factoryData = {
  badgeFA: "افتخار ملی، کیفیت جهانی", badgeEN: "National Pride, Global Quality",
  title1FA: "تولید با", title1EN: "Manufactured with",
  title2FA: "استاندارد", title2EN: "Standard",
  title3FA: "آلمان", title3EN: "German Quality",
  descFA: "ما در جزیره گندم با بهره‌گیری از خطوط تولید پیشرفته و فرمولاسیون اختصاصی، محصولاتی را خلق می‌کنیم که نه‌تنها طعمی بی‌نظیر دارند، بلکه تضمین‌کننده سلامت مصرف‌کنندگان هستند.",
  descEN: "At Jazireh Gandom, using advanced production lines and proprietary formulation, we create products with unique taste and health guarantees.",
  features: [
    { icon: Globe, fa: "توزیع گسترده", en: "Global Distribution", descFA: "صادرات به ۱۵ کشور", descEN: "Export to 15 countries" },
    { icon: Leaf, fa: "طعم طبیعی", en: "Natural Taste", descFA: "بدون مواد نگهدارنده", descEN: "No preservatives" },
    { icon: ShieldCheck, fa: "کیفیت تضمینی", en: "Quality Assured", descFA: "آزمایشگاه کنترل کیفی", descEN: "Quality Control Lab" },
    { icon: Factory, fa: "تکنولوژی مدرن", en: "Modern Tech", descFA: "خطوط تولید تمام اتوماتیک", descEN: "Fully automatic lines" },
  ],
  stats: [
    { val: 50, fa: "محصول متنوع", en: "Diverse Products" },
    { val: 1000, fa: "مشتری سازمانی", en: "B2B Clients" },
    { val: 20, fa: "سال تجربه", en: "Years Experience" },
    { val: 15, fa: "کشور صادراتی", en: "Export Countries" }
  ]
};

const whyUs = [
  { id: 1, titleFA: "کیفیت ارگانیک", titleEN: "Organic Quality", icon: Leaf },
  { id: 2, titleFA: "فرمولاسیون آلمانی", titleEN: "German Formula", icon: Zap },
  { id: 3, titleFA: "پخش گسترده", titleEN: "Wide Distribution", icon: Activity },
];

const partners = [
  { id: 1, name: "Partner A", logo: "https://placehold.co/200x100/f8fafc/0f172a?text=Logo+A", url: "#" },
  { id: 2, name: "Partner B", logo: "https://placehold.co/200x100/f8fafc/0f172a?text=Logo+B", url: "#" },
  { id: 3, name: "Partner C", logo: "https://placehold.co/200x100/f8fafc/0f172a?text=Logo+C", url: "#" },
];

const certificates = [
  { id: 1, name: "ISO 9001", img: "https://placehold.co/600x800/eab308/ffffff?text=ISO+9001" },
  { id: 2, name: "Halal", img: "https://placehold.co/600x800/10b981/ffffff?text=Halal" },
];

const mainBranch = {
  faName: "دفتر مرکزی جزیره گندم", enName: "Jazireh Gandom HQ", phone: "+93 79 522 4882", email: "info@jazirehgandom.com",
  wa: "93795224882", tg: "jazirehgandom", fb: "jazirehgandom", ig: "jazirehgandom",
  faAddress: "هرات، شهرک صنعتی، فاز 2، شقایق 3", enAddress: "Tehran, Valiasr St, Trade Tower, 10th Fl", mapUrl: "#"
};

const branches = [
  { id: 1, faName: "نمایندگی کابل", enName: "Kabul Branch", phone: "0795224882", email: "kabul@jazirehgandom.com", wa: "989150000000", tg: "jg_kabul", faAddress: "کابل، شهر نو، کوچه 13", enAddress: "Kabul, Sajjad Blvd", mapUrl: "#" },
];
...
const marqueeFA = { animate: { x: ["0%", "50%"], transition: { repeat: Infinity, duration: 40, ease: "linear" as const } } };
const marqueeEN = { animate: { x: ["0%", "-50%"], transition: { repeat: Infinity, duration: 40, ease: "linear" as const } } };
const marqueeRevFA = { animate: { x: ["50%", "0%"], transition: { repeat: Infinity, duration: 45, ease: "linear" as const } } };
const marqueeRevEN = { animate: { x: ["-50%", "0%"], transition: { repeat: Infinity, duration: 45, ease: "linear" as const } } };
...
export default function AboutPage() {
  ...
  // --- استیت برای نمایش پاپ‌آپ فرم نمایندگی ---
  ...
  const dupPartners = [...partners, ...partners, ...partners, ...partners, ...partners, ...partners, ...partners, ...partners];
  const dupCerts = [...certificates, ...certificates, ...certificates, ...certificates, ...certificates, ...certificates, ...certificates, ...certificates];
    ...
    <main className="w-full bg-transparent min-h-screen pt-28 pb-16 overflow-hidden" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-7xl flex flex-col gap-12">
        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.2 }} className="scroll-mt-28 flex flex-col items-center text-center">
          ...
          <motion.p variants={fadeInUp} className="text-lg text-gray-600 dark:text-gray-400 leading-relaxed max-w-4xl mx-auto text-justify md:text-center">{isRtl ? "شرکت جزیره گندم با بیش از دو دهه تجربه در تولید برترین برندهای نوشیدنی و تنقلات، همواره متعهد به ارائه بالاترین کیفیت بوده است. مسیر ما از یک رویای کوچک آغاز شد و امروز به یک خانواده بزرگ بین‌المللی تبدیل شده‌ایم." : "Jazireh Gandom Company, with over two decades of experience in producing top beverage and snack brands, has always been committed to providing the highest quality."}</motion.p>
        </motion.div>
        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.2 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl flex flex-col md:flex-row gap-5 hover:border-amber-400 transition-colors">
            ...
            <div>
              ...
              <p className="text-base text-gray-600 dark:text-gray-400 leading-relaxed text-justify">{isRtl ? "تأمین محصولات باکیفیت و ارگانیک برای ارتقای سطح سلامت جامعه، همراه با حفظ محیط زیست." : "Providing high-quality products to improve community health."}</p>
            </div>
          </motion.div>
          <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl flex flex-col md:flex-row gap-5 hover:border-amber-400 transition-colors">
            ...
            <div>
              ...
              <p className="text-base text-gray-600 dark:text-gray-400 leading-relaxed text-justify">{isRtl ? "تبدیل شدن به قطب تولید و صادرات محصولات غذایی سالم در خاورمیانه تا سال ۲۰۳۰." : "Becoming the largest hub for food export in the Middle East."}</p>
            </div>
          </motion.div>
        </motion.div>

        {/* ۳. تعهد به کیفیت */}
        <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.1 }} className="text-center bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl">
          ...
          <motion.p variants={fadeInUp} className="text-base text-gray-600 dark:text-gray-400 leading-relaxed max-w-4xl mx-auto">{isRtl ? "ما در جزیره گندم، کیفیت را نه یک مزیت، بلکه یک الزام می‌دانیم. تمامی محصولات ما تحت سخت‌گیرانه‌ترین استانداردهای کنترل کیفی بین‌المللی تولید می‌شوند." : "At Jazireh Gandom, we consider quality not an advantage, but a requirement. All our products are manufactured under the strictest standards."}</motion.p>
        </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:items-stretch items-center">
            <div className="flex flex-col h-full">
              <motion.div variants={fadeInUp} className="mb-6">
                <span className="inline-block px-4 py-1.5 mb-4 text-xs font-bold text-amber-700 bg-amber-400/20 rounded-full">{isRtl ? factoryData.badgeFA : factoryData.badgeEN}</span>
                <h2 className="text-3xl md:text-4xl font-black text-gray-900 dark:text-white mb-5 leading-tight">
                  {isRtl ? factoryData.title1FA : factoryData.title1EN} <span className="text-gray-900 dark:text-white">{isRtl ? factoryData.title2FA : factoryData.title2EN}</span> <span className="text-amber-500">{isRtl ? factoryData.title3FA : factoryData.title3EN}</span>
                </h2>
                <p className="text-base text-gray-600 dark:text-gray-400 leading-relaxed text-justify">{isRtl ? factoryData.descFA : factoryData.descEN}</p>
              </motion.div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                {factoryData.features.map((feature, i) => {
                  const Icon = feature.icon;
                  return (
                    <motion.div key={i} variants={fadeInUp} className="flex gap-3 items-center bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-4 rounded-2xl hover:border-amber-400 transition-colors">
                      <div className="shrink-0 w-12 h-12 bg-amber-400/10 rounded-xl flex items-center justify-center text-amber-500"><Icon size={24} /></div>
                      <div><h4 className="font-bold text-sm text-gray-900 dark:text-white mb-0.5">{isRtl ? feature.fa : feature.en}</h4><p className="text-xs text-gray-500">{isRtl ? feature.descFA : feature.descEN}</p></div>
                    </motion.div>
                  );
                })}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-auto">
                {factoryData.stats.map((s, i) => (<Counter key={i} value={s.val} title={isRtl ? s.fa : s.en} />))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {whyUs.map(item => (
              <motion.div key={item.id} variants={fadeInUp} className="bg-amber-400/5 border border-amber-400/20 p-6 rounded-3xl flex flex-col items-center text-center gap-4 text-amber-700 dark:text-amber-400 hover:bg-amber-400/10 transition-colors">
                <div className="p-4 bg-amber-400/20 rounded-full"><item.icon size={36} /></div>
                <span className="font-bold text-lg">{isRtl ? item.titleFA : item.titleEN}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {/* ۶. مشتریان و شرکا */}
        {visibilitySettings.showPartners && (
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.1 }}>
            ...
            <motion.div variants={fadeInUp} className="relative w-full overflow-hidden flex items-center" style={{ maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}>
              <motion.div animate={isRtl ? marqueeFA.animate : marqueeEN.animate} className="flex w-max gap-6 py-2">
                {dupPartners.map((partner, idx) => (
                  <a key={idx} href={partner.url} target="_blank" rel="noopener noreferrer" className="relative w-40 h-20 shrink-0 bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl flex items-center justify-center hover:border-amber-400 transition-all group/card">
                    <img src={partner.logo} alt={partner.name} className="max-w-24 max-h-10 opacity-60 group-hover/card:opacity-100 transition-opacity grayscale group-hover/card:grayscale-0" />
                  </a>
                ))}
              </motion.div>
            </motion.div>
        ...
        {/* ۷. گواهینامه‌ها */}
        {visibilitySettings.showCertificates && (
            ...
            <motion.div variants={fadeInUp} className="relative w-full overflow-hidden flex items-center" style={{ maskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 10%, black 90%, transparent)' }}>
              <motion.div animate={isRtl ? marqueeRevFA.animate : marqueeRevEN.animate} className="flex w-max gap-6 py-2">
                {dupCerts.map((cert, idx) => (
                  <div key={idx} onClick={() => setSelectedCert(cert.img)} className="w-32 h-44 shrink-0 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-2 cursor-pointer hover:border-amber-400 hover:scale-105 transition-all">
                    <img src={cert.img} alt={cert.name} className="w-full h-full object-cover rounded-xl" />
                  </div>
                ))}
              </motion.div>
            </motion.div>
        ...
        <motion.div id="contact" variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.1 }} className="scroll-mt-28">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <motion.div variants={fadeInUp} className="bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl h-full flex flex-col justify-between">
              <div>
            ...
            <motion.div variants={fadeInUp} className="bg-gray-900 text-white border border-gray-800 p-8 rounded-3xl relative overflow-hidden flex flex-col justify-between h-full">
              ...
              <div className="relative z-10">
                <h3 className="text-2xl font-black text-amber-400 mb-8 flex items-center gap-3"><Building2 size={28}/> {isRtl ? mainBranch.faName : mainBranch.enName}</h3>
                <div className="flex flex-col gap-6 mb-10">
                  <a href={`tel:${mainBranch.phone}`} className="flex items-center gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="p-3 bg-white/10 rounded-xl group-hover:bg-amber-400 group-hover:text-gray-900 transition-colors"><Phone size={20}/></div><span dir="ltr" className="font-mono font-medium text-xl">{mainBranch.phone}</span></a>
                  <a href={`mailto:${mainBranch.email}`} className="flex items-center gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="p-3 bg-white/10 rounded-xl group-hover:bg-amber-400 group-hover:text-gray-900 transition-colors"><Mail size={20}/></div><span className="font-mono text-base">{mainBranch.email}</span></a>
                  <a href={mainBranch.mapUrl} target="_blank" className="flex items-start gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="p-3 bg-white/10 rounded-xl group-hover:bg-amber-400 group-hover:text-gray-900 transition-colors shrink-0"><MapPinned size={20}/></div><span className="text-base leading-relaxed mt-1">{isRtl ? mainBranch.faAddress : mainBranch.enAddress}</span></a>
                </div>
              </div>
              <div className="relative z-10 w-full flex justify-between items-center bg-white/5 p-2 rounded-2xl border border-white/10">
                <a href={`https://wa.me/${mainBranch.wa}`} target="_blank" className="flex-1 flex justify-center py-2.5 hover:text-amber-400 transition-colors"><MessageCircle size={22}/></a>
                <a href={`https://t.me/${mainBranch.tg}`} target="_blank" className="flex-1 flex justify-center py-2.5 hover:text-amber-400 transition-colors"><Send size={22}/></a>
                <a href={`https://instagram.com/${mainBranch.ig}`} target="_blank" className="flex-1 flex justify-center py-2.5 hover:text-amber-400 transition-colors"><Instagram size={22}/></a>
                <a href={`https://facebook.com/${mainBranch.fb}`} target="_blank" className="flex-1 flex justify-center py-2.5 hover:text-amber-400 transition-colors"><Facebook size={22}/></a>
                <a href={mainBranch.mapUrl} target="_blank" className="flex-1 flex justify-center py-2.5 hover:text-amber-400 transition-colors"><MapPin size={22}/></a>
                <button onClick={() => downloadVCF(mainBranch)} className="md:hidden flex-1 flex justify-center py-2.5 text-amber-400 hover:text-white transition-colors"><Download size={22}/></button>
              </div>
            </motion.div>
          </div>
        </motion.div>
        ...
        <motion.div id="branches" variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.1 }} className="scroll-mt-28 mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            {branches.map((branch) => (
              <motion.div key={branch.id} variants={fadeInUp} className="bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-8 rounded-3xl flex flex-col justify-between hover:border-amber-400 transition-colors">
          ...
          {/* دکمه باز کردن پاپ‌آپ درخواست نمایندگی */}
