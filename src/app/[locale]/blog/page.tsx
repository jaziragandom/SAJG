...
import { Clock, Calendar, ArrowUpLeft, ArrowUpRight, Loader2 } from "lucide-react";
...
export default function BlogListPage() {
  ...
  return (
    <main className="min-h-screen bg-transparent pt-32 pb-24 px-6 md:px-12 lg:px-20 max-w-6xl mx-auto" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="mb-16 text-center overflow-hidden py-4">
        ...
        <motion.p {...descAnim} className="text-gray-600 dark:text-gray-300 font-medium max-w-2xl mx-auto">
          {isRtl 
            ...
            : "Stories, news, and specialized articles from the world of beverages and a healthy lifestyle."}
        </motion.p>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-gray-500 font-bold">{isRtl ? "در حال دریافت مقالات..." : "Loading articles..."}</p>
        </div>
        ...
        >
          <p className="text-gray-500 dark:text-gray-400 font-bold text-sm">
            {isRtl ? "مقاله‌ای در این دسته‌بندی یافت نشد." : "No articles found in this category."}
          </p>
        </motion.div>
      ) : (
        <>
            ...
            >
              <Link href={`/${locale}/blog/${featuredPost.slug}`} className="group relative flex flex-col md:flex-row bg-white/60 dark:bg-gray-950/60 backdrop-blur-2xl border border-white/20 dark:border-gray-800/50 rounded-[2.5rem] overflow-hidden shadow-2xl hover:shadow-amber-500/10 transition-all duration-500">
                <div className="w-full md:w-3/5 h-64 md:h-112 relative overflow-hidden shrink-0">
                 <Image src={featuredPost.coverImage} alt={isRtl ? featuredPost.faTitle : featuredPost.enTitle} fill sizes="(max-width: 768px) 100vw, 60vw" className="object-cover group-hover:scale-105 transition-transform duration-700 ease-out" />

                  <div className={`absolute top-6 ${isRtl ? 'right-6' : 'left-6'} bg-amber-400 text-gray-950 text-xs font-black px-4 py-2 rounded-full shadow-lg`}>
                ...
                <div className="p-8 md:p-12 flex flex-col justify-center w-full">
                  <div className="flex items-center gap-4 text-xs font-bold text-gray-500 dark:text-gray-400 mb-4">
                    <span className="flex items-center gap-1.5"><Calendar size={14}/> {new Date(featuredPost.createdAt).toLocaleDateString(isRtl ? 'fa-IR' : 'en-US')}</span>
                  ...
                  <h2 className="text-2xl md:text-4xl font-black text-gray-900 dark:text-white mb-4 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors leading-tight">
                    {isRtl ? featuredPost.faTitle : featuredPost.enTitle}
                  </h2>
                  ...
                  <div className="mt-auto flex items-center gap-2 text-amber-600 dark:text-amber-400 font-black text-sm group-hover:gap-4 transition-all">
                    {isRtl ? "مطالعه کامل مقاله" : "Read full article"} 
              ...
              >
                <Link href={`/${locale}/blog/${post.slug}`} className="flex flex-col h-full bg-white/60 dark:bg-gray-950/60 backdrop-blur-2xl border border-white/20 dark:border-gray-800/50 rounded-3xl overflow-hidden group hover:shadow-xl hover:-translate-y-1 transition-all duration-500">
                  <div className="h-48 relative overflow-hidden shrink-0">
                    <img src={post.thumbnailImage} alt={isRtl ? post.faTitle : post.enTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 ease-out" />
                    <div className={`absolute top-4 ${isRtl ? 'right-4' : 'left-4'} bg-white/90 dark:bg-gray-900/90 backdrop-blur-md text-gray-900 dark:text-white text-[10px] font-black px-3 py-1.5 rounded-full`}>
                      {post.category === 'health' ? (isRtl ? 'سلامت' : 'Health') : post.category === 'news' ? (isRtl ? 'اخبار' : 'News') : (isRtl ? 'محصولات' : 'Products')}
                    </div>
                  </div>
                  <div className="p-6 flex flex-col grow">
                    <h3 className="text-lg font-black text-gray-900 dark:text-white mb-3 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors line-clamp-2 leading-snug">
                      {isRtl ? post.faTitle : post.enTitle}
                    </h3>
                    ...
                    <div className="mt-auto flex items-center justify-between text-[11px] font-bold text-gray-500 dark:text-gray-400 border-t border-gray-200/50 dark:border-gray-800/50 pt-4">
                      <span className="flex items-center gap-1.5"><Calendar size={14}/> {new Date(post.createdAt).toLocaleDateString(isRtl ? 'fa-IR' : 'en-US')}</span>
