...
import { Plus, Edit3, Trash2, GripVertical, X, Upload, CheckCircle2, Wand2, Loader2, Image as ImageIcon, Sparkles, RefreshCcw } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const initialSlides = [
  { id: 1, faTitle: "انرژی بی‌نهایت", enTitle: "Infinite Energy", faSubtitle: "جدیدترین محصول", enSubtitle: "New Arrival", color: "from-amber-400 to-orange-600" },
  { id: 2, faTitle: "طعم واقعی انار", enTitle: "Real Pomegranate Taste", faSubtitle: "محصول ارگانیک", enSubtitle: "Organic Product", color: "from-red-500 to-rose-700" },
  { id: 3, faTitle: "تنوع بی‌نظیر اسنک", enTitle: "Unmatched Snack Variety", faSubtitle: "طعم‌های جدید", enSubtitle: "New Flavors", color: "from-yellow-300 to-amber-500" },
];
...
export default function HeroManager() {
  ...
  const handleEdit = (slide: any) => {
    ...
    setFormData({ 
      ...formData, 
      id: slide.id, faTitle: slide.faTitle, enTitle: slide.enTitle, 
      faSubtitle: slide.faSubtitle, enSubtitle: slide.enSubtitle, color: slide.color 
    });
    setUploadedFloaters([]); // ریست کردن برای پیش‌نمایش جدید
    setActiveTab("texts");
    setIsModalOpen(true);
  };
  ...
  const handleAddNew = () => {
    setEditMode(false);
    setFormData({ 
      id: 0, faTitle: "", enTitle: "", faSubtitle: "", enSubtitle: "", faDesc: "", enDesc: "", color: "from-amber-400 to-orange-600"
    });
    setUploadedFloaters([]);
    setActiveTab("texts");
    setIsModalOpen(true);
  ...
  // --- شبیه‌ساز الگوریتم هوشمند تکثیر و تنظیم افکت قطعات معلق ---
  const handleSimulateFloaterUpload = () => {
    setIsProcessingFloaters(true);
    
    setTimeout(() => {
      // فرض کنیم ادمین ۳ عکس آپلود کرده است
      const baseUploads = [
        { id: 1, type: "slice_1" },
        { id: 2, type: "slice_2" },
        { id: 3, type: "drop_1" }
      ...
      // الگوریتم تکثیر تا ۸ عدد با اعمال افکت‌های تصادفی
      ...
      const finalFloaters = [];
      for (let i = 0; i < 8; i++) {
        const baseItem = baseUploads[i % baseUploads.length]; // چرخش بین آیتم‌های آپلود شده
        finalFloaters.push({
          ...baseItem,
          uniqueId: Date.now() + i,
        ...
        <div className="flex flex-col gap-3">
          {slides.map((slide, index) => (
            <div
              ...
              className={`bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 flex items-center gap-4 group shadow-sm transition-colors ${draggedItemIndex === index ? 'opacity-50' : 'hover:border-amber-400'}`}
            >
              ...
              <div className={`w-14 h-14 rounded-xl bg-linear-to-br ${slide.color} shadow-inner flex items-center justify-center`}>
              ...
              <div className="flex items-center gap-2 pl-2">
                <button onClick={() => handleEdit(slide)} className="p-2 bg-white dark:bg-gray-900 text-gray-400 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-xl transition-colors" title="ویرایش">
                  <Edit3 size={18} />
                </button>
                <button onClick={() => handleDelete(slide.id)} className="p-2 bg-white dark:bg-gray-900 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-colors" title="حذف">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6">
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900">
                ...
                <button onClick={() => setIsModalOpen(false)} className="p-2 text-gray-400 hover:text-red-500 bg-gray-100 dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
                ...
                {activeTab === "texts" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input type="text" value={formData.faSubtitle} onChange={e => setFormData({...formData, faSubtitle: e.target.value})} placeholder="مثال: جدیدترین محصول" className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-400" />
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-gray-600 dark:text-gray-400">Subtitle (English)</label>
                      <div className="relative">
                        <input type="text" dir="ltr" value={formData.enSubtitle} onChange={e => setFormData({...formData, enSubtitle: e.target.value})} placeholder="New Arrival" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 pr-4 pl-12 text-sm font-mono focus:outline-none focus:border-amber-400" />
                        <button type="button" onClick={() => handleAutoTranslate(formData.faSubtitle, 'enSubtitle')} disabled={translatingField === 'enSubtitle' || !formData.faSubtitle} className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-amber-400/10 text-amber-600 hover:bg-amber-400 hover:text-gray-950 disabled:opacity-50 rounded-lg transition-colors">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input type="text" value={formData.faTitle} onChange={e => setFormData({...formData, faTitle: e.target.value})} placeholder="مثال: انرژی بی‌نهایت" className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-400" />
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-gray-600 dark:text-gray-400">Main Title (English)</label>
                      <div className="relative">
                        <input type="text" dir="ltr" value={formData.enTitle} onChange={e => setFormData({...formData, enTitle: e.target.value})} placeholder="Infinite Energy" className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 pr-4 pl-12 text-sm font-mono focus:outline-none focus:border-amber-400" />
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <textarea rows={3} value={formData.faDesc} onChange={e => setFormData({...formData, faDesc: e.target.value})} placeholder="توضیحات زیر تیتر اصلی..." className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-amber-400 resize-none"></textarea>
                    </div>
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-gray-600 dark:text-gray-400">Description (English)</label>
                      <div className="relative">
                        <textarea rows={3} dir="ltr" value={formData.enDesc} onChange={e => setFormData({...formData, enDesc: e.target.value})} placeholder="Description text..." className="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 pr-4 pl-12 text-sm font-mono focus:outline-none focus:border-amber-400 resize-none"></textarea>
                      ...
                      <div className="flex gap-4">
                        {[
                          ...
                          <button 
                            ...
                            className={`w-12 h-12 rounded-full ${c.bg} shadow-md transition-transform border-4 ${formData.color === c.val ? 'border-gray-900 dark:border-white scale-110' : 'border-transparent hover:scale-105'}`}
                          />
                        ))}
                      </div>
                    </div>
                ...
                {activeTab === "images" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl h-44 flex flex-col items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800/50 dark:hover:bg-gray-800 transition-colors cursor-pointer group">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-4 h-44">
                        <div className="flex-1 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-2xl flex flex-col items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800/50 dark:hover:bg-gray-800 transition-colors cursor-pointer group p-2 text-center">
                    ...
                    <div className="flex flex-col gap-2 md:col-span-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                      ...
                      <p className="text-xs font-medium text-gray-500 leading-relaxed mb-3 pr-6 border-r-2 border-amber-400">
                        تصاویری مثل قاچ‌های میوه، برگ، قطرات آب یا اسنک‌های کوچک را آپلود کنید. 
                        <strong>(ابعاد پیشنهادی: ۱۵۰x۱۵۰ پیکسل - فرمت PNG)</strong><br/>
                      ...
                      <div className="flex items-stretch gap-4">
                        ...
                        <button 
                          type="button"
                          onClick={handleSimulateFloaterUpload}
                          disabled={isProcessingFloaters}
                          className="w-40 border-2 border-dashed border-amber-300 dark:border-amber-700/50 rounded-2xl flex flex-col items-center justify-center gap-3 bg-amber-50/50 hover:bg-amber-50 dark:bg-amber-900/10 dark:hover:bg-amber-900/20 transition-colors group p-4 shrink-0"
                        >
