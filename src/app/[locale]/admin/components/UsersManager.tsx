...
import { Plus, Search, Edit3, Trash2, X, CheckCircle2, ShieldAlert, Shield, ShieldHalf, User, KeyRound, Mail, ToggleLeft, ToggleRight, Check, Clock } from "lucide-react";
...
export default function UsersManager() {
  ...
  const filteredUsers = users.filter(u => {
    const matchesSearch = u.name.includes(searchQuery) || u.email.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleEditOpen = (item: any) => {
    ...
    setFormData({
      ...
      role: item.role, status: item.status, permissions: item.permissions.includes("all") ? availablePermissions.map(p=>p.id) : item.permissions
    });
    setIsModalOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    const newUserData = {
      ...
      status: formData.status, permissions: finalPermissions,
      avatar: editItem?.avatar || `https://placehold.co/150x150/1e293b/ffffff?text=${formData.name.charAt(0)}`
    };

    if (editItem) {
      setUsers(prev => prev.map(p => p.id === editItem.id ? { ...p, ...newUserData } : p));
    } else {
      const newUser = { id: Date.now(), lastLogin: "ثبت‌نام جدید", ...newUserData };
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto flex flex-col gap-8" dir="rtl">
      ...
      {/* فیلترها و سرچ */}
      <div className="flex flex-col md:flex-row gap-4 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200/60 dark:border-gray-800/60 p-2 shadow-xs shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence mode="popLayout">
          {filteredUsers.map(user => (
            <motion.div key={user.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.4, ease: customEase }} className={`bg-white dark:bg-gray-900 border ${user.status === 'active' ? 'border-gray-200 dark:border-gray-800' : 'border-red-200 dark:border-red-900/50 opacity-75'} rounded-[2rem] p-6 flex flex-col gap-5 shadow-sm hover:shadow-xl transition-all`}>
              
              <div className="flex justify-between items-start">
                <div className="flex gap-4 items-center">
                  <img src={user.avatar} alt={user.name} className="w-14 h-14 rounded-2xl object-cover shadow-inner" />
              ...
              <div className="flex flex-col gap-3 bg-gray-50 dark:bg-gray-800/40 p-4 rounded-2xl">
                ...
                <span className="flex items-center gap-2 text-xs font-bold text-gray-600 dark:text-gray-400" dir="ltr"><Clock size={14} className="text-gray-400"/> {user.lastLogin} :Login</span>
              </div>
              ...
              <div className="mt-auto pt-2 flex gap-2 justify-end">
                ...
                <button onClick={() => handleDelete(user.id, user.role)} className="p-3 bg-gray-50 dark:bg-gray-800 text-gray-400 hover:text-white hover:bg-red-500 rounded-xl transition-colors"><Trash2 size={16} /></button>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
